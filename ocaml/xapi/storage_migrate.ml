...
let copy' ~task ~sr ~vdi ~url ~dest ~dest_vdi =
	...
	let vdis = Remote.SR.scan ~task ~sr |> success |> _vdis in
	let remote_vdi = 
	...
	try
		...
		let base_vdi = 
			try Some (Local.VDI.get_by_name ~task ~sr:dest ~name:dest_content_id |> success |> _vdi).vdi
			with _ -> None
		in
...
let start ~task ~sr ~vdi ~dp ~url ~dest =
	...
	try
		...
		let uri = (Printf.sprintf "/services/SM/nbd/%s/%s/%s" sr result.mirror_vdi.vdi mirror_dp) in
		let dest_url = Http.Url.set_uri remote_url uri in
		let request = Http.Request.make ~user_agent:"smapiv2" Http.Put uri in
		...
		debug "Got it!";
		ignore(match Storage_impl.tapdisk_of_path params with 
			| Some tapdev -> 
...
let receive_start ~task ~sr ~vdi_info ~content_id ~similar =
	...
	let parent_dp = Local.DP.create ~task ~id:(Uuid.string_of_uuid (Uuid.make_uuid ())) in
	...
	try
		...
		let _ = Local.VDI.attach ~task ~dp:parent_dp ~sr ~vdi:parent.vdi 
			~read_write:true |> success |> params in

		Local.VDI.activate ~task ~dp:parent_dp ~sr ~vdi:parent.vdi |> success |> unit;
