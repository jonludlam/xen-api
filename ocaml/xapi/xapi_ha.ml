...
let i_have_statefile_access () =
  ...
  with e ->
    info "Caught exception querying liveset; assuming we have no statefile access: %s"
...
let write_uuid_to_ip_mapping ~__context =
  let table = List.map
      (fun (_, host) ->
         host.API.host_uuid, host.API.host_address)
...
module Timeouts = struct
  ...
  let derive (t: int) =
    ...
    { heart_beat_interval = heart_beat_interval;
      ...
      xapi_restart_timeout = !Xapi_globs.ha_xapi_restart_timeout;
  ...
  let get_base_t ~__context =
    ...
    (* We then allowed it to be bumped persistently by users to work around multipath breakage *)
    (* in CA-28306 *)
    let t =
...
module Monitor = struct
  ...
  let ha_monitor () : unit = Debug.with_thread_named "ha_monitor" (fun () ->
      ...
      Server_helpers.exec_with_new_task "HA monitor" (fun __context ->
          ...
          let warning_heartbeat_approaching_timeout =
            boolean_warning Api_messages.ha_heartbeat_approaching_timeout None in
          let warning_statefile_approaching_timeout =
            boolean_warning Api_messages.ha_statefile_approaching_timeout None in
          let warning_xapi_healthcheck_approaching_timeout =
            boolean_warning Api_messages.ha_xapi_healthcheck_approaching_timeout None in
          ...
          let warning_network_bonding_error =
            boolean_warning Api_messages.ha_network_bonding_error
              (Some (Printf.sprintf
                       "The network bond used for transmitting HA heartbeat messages on host '%s' has failed"
                       localhost_uuid)) in
          ...
          let query_liveset_on_all_hosts () =
            ...
            Opt.iter
              (fun local ->
                 ...
                 let open Xha_interface.LiveSetInformation.RawStatus in
                 let statefile = float_of_int local.statefile_latency /. 1000. in
                 let heartbeat_latency = float_of_int local.heartbeat_latency /. 1000. -.
                                         (float_of_int timeouts.Timeouts.heart_beat_interval) in
                 let xapi_latency = float_of_int (local.xapi_healthcheck_latency) /. 1000. in
                 let statefile_latencies = List.map
                     (fun vdi ->
            ...
            Opt.iter
              (fun warning ->
                 let open Xha_interface.LiveSetInformation.Warning in
                 warning_statefile_lost warning.statefile_lost;
                 warning_heartbeat_approaching_timeout warning.heartbeat_approaching_timeout;
                 warning_statefile_approaching_timeout warning.statefile_approaching_timeout;
                 warning_xapi_healthcheck_approaching_timeout warning.xapi_healthcheck_approaching_timeout;
                 warning_network_bonding_error warning.network_bonding_error;
          ...
          let process_liveset_on_slave liveset =
            ...
            let master_info = Hashtbl.find liveset.Xha_interface.LiveSetInformation.hosts
                (Uuid.uuid_of_string master_uuid) in
            if true
          ...
          let process_liveset_on_master liveset =
            let pool = Helpers.get_pool ~__context in
            let to_tolerate = Int64.to_int
                (Db.Pool.get_ha_host_failures_to_tolerate ~__context ~self:pool) in
            ...
            if local_failover_decisions_are_ok () then begin
              ...
              with e ->
                error "Caught unexpected exception when executing restart plan: %s"
                  (ExnHelper.string_of_exn e)
            end;
                ...
                Helpers.log_exn_continue
                  ...
                  (fun () ->
                     ...
                     if current <> live then begin
                       (* This can only be a false -> true transient as the 'restart_auto_run_vms'
                          function has already dealt with the true -> false case. *)
                       (* => live must be true. No need to consider calling current script hooks *)
                       if shutting_down
                       then info "Not marking host %s as live because it is shutting down"
                           (Ref.string_of host)
                       else begin
            ...
            (* If all live hosts have lost statefile then we are running thanks to Survival Rule 2:
               ...
               hosts can see the storage but others cannot, there is no point generating an alert
               because those who cannot are about to fence. *)
            let relying_on_rule_2 xha_host =
              true
              && xha_host.Xha_interface.LiveSetInformation.Host.liveness
              (* it is still alive *)
              && not xha_host.Xha_interface.LiveSetInformation.Host.state_file_access
              (* and yet has no statefile access *)
            in
            let all_live_nodes_lost_statefile =
              List.fold_left (&&) true
                (List.map (fun (_, xha_host) -> relying_on_rule_2 xha_host) host_host_table) in
            ...
            begin match liveset.Xha_interface.LiveSetInformation.raw_status_on_local_host with
              ...
              | Some local ->
                ...
                List.iter (fun (host, raw) ->
                    (* Use the list of network peers given by the host recent update: statefile or
                       network *)
                    let open Xha_interface.LiveSetInformation.HostRawData in
                    let peers =
                      if raw.time_since_last_update_on_statefile < raw.time_since_last_heartbeat
                      then raw.heartbeat_active_list_on_statefile
                      else raw.heartbeat_active_list_on_heartbeat in
            ...
            if plan_too_old || !plan_out_of_date then begin
              ...
              if changed && not !plan_out_of_date
              then error "Overcommit protection failed to prevent a change which invalidated our failover plan";

              last_plan_time := now;
              plan_out_of_date := false;
            end in

          (* Wait for all hosts in the liveset to become enabled so we can start VMs on them. We wait
             for up to ha_monitor_startup_timeout. *)
          let wait_for_slaves_on_master () =
            (* CA-17849: need to make sure that, in the xapi startup case, all hosts have been set to
               dead and disabled initially *)
            ...
            info "Master HA startup waiting for up to %.2f for slaves in the liveset to report in and enable themselves"
            ...
            while Mutex.execute m (fun () -> not(!request_shutdown)) && not(!finished) do
              try
                ...
                if Mutex.execute m (fun () -> not(!request_shutdown)) then begin
                  ...
                  let enabled = List.map
                      (fun uuid -> Db.Host.get_enabled
                          ~__context
                          ~self:(Db.Host.get_by_uuid ~__context ~uuid))
                      uuids in
                  let enabled, disabled = List.partition
                      (fun (_, x) -> x) (List.combine uuids enabled) in
                  ...
                  if disabled = [] then begin
                    info "Master HA startup: All live hosts are now enabled: [ %s ]"
                      (String.concat "; " (List.map fst enabled));
                    finished := true;
                  end;

                  if Unix.gettimeofday () -. start >
                     !Xapi_globs.ha_monitor_startup_timeout && disabled <> [] then begin
              ...
              with e ->
                debug "Exception in HA monitor thread while waiting for slaves: %s"
                  (ExnHelper.string_of_exn e);
                Thread.delay !Xapi_globs.ha_monitor_interval
            done in

          (* If we're the master we must wait for our live slaves to turn up before we
             consider restarting VMs etc *)
            ...
            try
              ...
              if Mutex.execute m (fun () -> not(!request_shutdown)) then begin
                ...
                if Pool_role.is_master () then begin
                  (* CA-23998: allow MTC to block master failover actions (ie VM restart)
                     for a certain period of time while their Level 2 VMs are being restarted. *)
                  finally
                    (fun () ->
                       let until = Mutex.execute m
                           (fun () ->
                              (* Callers of 'delay' will have to wait until we have finished
                                 processing this loop iteration *)
                       ...
                       if now < until
                       then
                         debug "Blocking VM restart thread for at least another %.0f seconds"
                           (until -. now)
  ...
  let start () =
    ...
    (* NB in the xapi startup case this will prevent the db_gc.single_pass from setting any
       live flags *)
    Mutex.execute Db_gc.use_host_heartbeat_for_liveness_m
    ...
    Mutex.execute thread_m
      (fun () ->
         ...
         | None ->
           (* This will cause the started thread to block until signal_database_state_valid
              is called *)
...
let redo_log_ha_enabled_at_startup () =
  (* If we are still the master, extract any HA metadata database so we can consider
     population from it *)
  if Pool_role.is_master () then begin
...
let on_server_restart () =
  ...
  if armed then begin
    ...
    while not (!finished) do
      ...
      (* If someone has called Host.emergency_ha_disable in the background then we notice the
         change here *)
      ...
      try
        if Xapi_fist.ha_cannot_access_statefile ()
        then raise (Xha_error Xha_errno.Mtc_exit_can_not_access_statefile);

        if Xapi_fist.ha_daemon_startup_failed ()
        then failwith "simulating xha daemon startup failure";
      ...
      | Xha_error Xha_errno.Mtc_exit_can_not_access_statefile as e ->
        ...
        (* check the Pool.ha_enabled on the master... assuming we can find the master. If we
    ...
    Monitor.start ();
    (* We signal the monitor that the database state is valid (wrt liveness + disabledness
       of hosts) later *)
  end
  ...
  Server_helpers.exec_with_new_task "Setting all hosts to dead and disabled"
    (fun __context ->
       List.iter (fun self ->
           ...
           info "Host.enabled: on_database_engine_ready: setting host %s (%s) to disabled"
             uuid hostname;
...
let emergency_ha_disable __context soft =
  ...
  if not ha_armed then
    ...
    begin
      ...
      with Xha_error e ->
        error "Host.emergency_ha_disable: ha_disarm_fencing failed with %s; continuing"
    ...
    begin
      ...
      with Xha_error e ->
        error "Host.emergency_ha_disable: ha_stop_daemon failed with %s; continuing"
          (Xha_errno.to_string e)
    end;
    ...
    begin
      ...
      with Xha_error e ->
        error "Host.emergency_ha_disable: ha_set_excluded failed with %s; continuing"
          (Xha_errno.to_string e)
    end;
    ...
    try
      List.iter
        (fun vdi ->
           ...
           paths :=
             Static_vdis.permanent_vdi_attach
               ~__context
               ~vdi
               ~reason:Xha_statefile.reason :: !paths) statevdis
    with e ->
...
let write_config_file ~__context statevdi_paths generation =
  ...
  (* Need to find the name of the physical interface, so xHA can monitor the bonding status
     (if appropriate). Note that this interface isn't used for sending packets so VLANs don't
     matter: the physical NIC or bond device is all we need. *)
  let localhost = Helpers.get_localhost ~__context in
  let mgmt_pifs =
    List.filter
      (fun self -> Db.PIF.get_management ~__context ~self)
      (Db.Host.get_PIFs ~__context ~self:localhost) in
  if mgmt_pifs = []
  then failwith (Printf.sprintf "Cannot enable HA on host %s: there is no management interface for heartbeating"
...
let join_liveset __context host =
  ...
  if Pool_role.is_master () then begin
    ...
    while not !master_found do
      ...
      let liveset = query_liveset () in
      debug "Liveset: %s" (Xha_interface.LiveSetInformation.to_summary_string liveset);
      if liveset.Xha_interface.LiveSetInformation.status =
         Xha_interface.LiveSetInformation.Status.Online then begin
...
let commit_new_master ~__context ~address =
  begin match !proposed_master with
    | Some x when x <> address ->
      let msg = Printf.sprintf "Received commit_new_master(%s) but previously received proposal for %s"
          address x in
      error "%s" msg;
...
let disable_internal __context =
  ...
  let metadata_vdis =
    List.map
      ...
      (Xha_metadata_vdi.list_existing ()) in
  ...
  let attempt_disable_through_statefile () =
    ...
    begin
      ...
      with e ->
        error "Caught exception %s while setting pool state to invalid-- retrying with no-statefile procedure"
          (ExnHelper.string_of_exn e);
        raise e
    end;
    ...
    Helpers.call_api_functions ~__context
      (fun rpc session_id ->
         ...
         List.iter
           (fun (host, e) ->
              error "Host '%s' ('%s') failed to diable HA via statefile; if node has statefile access it will disarm; if not it will self-fence (%s)"
                (Db.Host.get_name_label ~__context ~self:host)
                (Ref.string_of host)
                (ExnHelper.string_of_exn e)
           ) errors
  ...
  let attempt_disable_without_statefile () =
    ...
    Helpers.call_api_functions ~__context
      (fun rpc session_id ->
         ...
         List.iter
           (fun (host, e) ->
              error "Failed to disarm fencing on host '%s' ('%s'); this means the host may well be about to fence itself even though HA is officially disabled on the Pool (%s)"
                (Db.Host.get_name_label ~__context ~self:host)
                (Ref.string_of host)
                (ExnHelper.string_of_exn e)
           ) errors;
...
let enable __context heartbeat_srs configuration =
  ...
  (* Check also that any PIFs with IP information set are currently attached - it's
     a non-fatal error if they are, but we'll warn with a message *)
  let pifs_with_ip_config = Db.PIF.get_records_where ~__context ~expr:(
      Not (Eq (Field "ip_configuration_mode", Literal "None"))
    ) in
  let not_bond_slaves =
    List.filter
      (fun (_,pifr) -> not (Db.is_valid_ref __context pifr.API.pIF_bond_slave_of))
      pifs_with_ip_config in
  let without_disallow_unplug =
    List.filter
      (fun (_,pifr) -> not (pifr.API.pIF_disallow_unplug || pifr.API.pIF_management))
      not_bond_slaves in
  if List.length without_disallow_unplug > 0 then begin
    let pifinfo =
      List.map
        ...
        without_disallow_unplug in
    let bodylines =
      ["A possible network anomaly was found. The following hosts possibly have storage PIFs that are not dedicated:"] @
      (List.map (fun (hostname,pif,pifr) ->
           Printf.sprintf "%s: %s (uuid: %s)" hostname pifr.API.pIF_device pifr.API.pIF_uuid) pifinfo)
    in
    ...
    ignore(
      Xapi_message.create
        ...
        ~priority
        ~cls:`Pool
        ~obj_uuid:(Db.Pool.get_uuid ~__context ~self:(Helpers.get_pool ~__context))
        ~body:(String.concat "\n" bodylines))
  end;
  ...
  List.iter (fun host ->
      let alive =
        ...
        with _ -> false in
      if not alive
      then raise (Api_errors.Server_error(Api_errors.host_offline, [ Ref.string_of host ]))
    ) (Db.Host.get_all ~__context);
  ...
  try
    ...
    let possible_srs =
      if heartbeat_srs = []
      then Xha_statefile.list_srs_which_can_host_statefile ~__context ~cluster_stack
      else heartbeat_srs in
    if List.length possible_srs = 0
    then raise (Api_errors.Server_error(Api_errors.cannot_create_state_file, []));

    (* For the moment we'll create a state file in one compatible SR since the xHA
       component only handles one *)
    ...
    Helpers.call_api_functions ~__context
      (fun rpc session_id ->
         (* ... *)
         (* Tell each host to attach its statefile, write config files etc. Do not start
            the xHA daemon. *)
         List.iter
           (fun host ->
              try
                ...
                count_call ()
              with e ->
                error "Caught exception while calling Host.preconfigure_ha: '%s' ('%s') %s"
                  (Db.Host.get_name_label ~__context ~self:host)
                  (Ref.string_of host)
                  (ExnHelper.string_of_exn e);
         ...
         List.iter
           (fun (host, e) ->
              error "Caught exception while calling Host.ha_join_liveset: '%s' ('%s') %s"
                (Db.Host.get_name_label ~__context ~self:host)
                (Ref.string_of host)
                (ExnHelper.string_of_exn e)
           ) errors;
         ...
         List.iter
           (fun (host, e) ->
              error "Caught exception while calling Host.request_backup: '%s' ('%s') %s"
                (Db.Host.get_name_label ~__context ~self:host)
                (Ref.string_of host)
                (ExnHelper.string_of_exn e)
           ) errors;
         if errors <> [] then begin
           ...
           error "Attempting to disable HA pool-wide";
           Helpers.log_exn_continue
             "Disabling HA after a failure during enable"
             disable_internal
             __context;
           raise (snd (List.hd errors))
         end;
         ...
         List.iter (fun vdi -> Xapi_vdi.update_allowed_operations ~__context ~self:vdi)
  ...
  if Db.Pool.get_ha_enabled ~__context ~self:pool then begin
    ...
    begin try
      ...
      with e ->
        ...
        error "Error past the commit-point while cleanly shutting down host: %s"
          (ExnHelper.string_of_exn e);
