...
type vdi_mirror = {
  ...
  dp : string;                        (* The datapath the VDI may or may not currently be using *)
...
let migrate_send'  ~__context ~vm ~dest ~live ~vdi_map ~vif_map ~options =
  ...
  with e ->
    ...
    | Some loc -> 
      let vconf = List.find (fun vconf -> vconf.location=loc) vms_vdis in
